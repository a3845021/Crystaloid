...
Scene* Crystaloid::createScene()
{
    return Crystaloid::create();
}


bool Crystaloid::init()
{
    if ( !Scene::init() )
        return false;
	...
	SimpleAudioEngine::getInstance()->preloadEffect("explode.mp3");
	...
	listenerT->onTouchBegan = CC_CALLBACK_2(Crystaloid::onTouchStart, this);
	listenerT->onTouchMoved = CC_CALLBACK_2(Crystaloid::onTouchMove, this);
	listenerT->onTouchEnded = CC_CALLBACK_2(Crystaloid::onTouchEnd, this);
...
bool Crystaloid::onTouchStart(Touch *touch, Event *event)
{
...
bool Crystaloid::onTouchMove(Touch *touch, Event *event)
{
	if (player.life > 0 && pseudoScene == GAME)
	{
		if (touch->getLocation().x - player.sprite->getContentSize().width / 2 >= 0 && touch->getLocation().x + player.sprite->getContentSize().width / 2 <= visibleSize.width)
...
bool Crystaloid::onTouchEnd(Touch *touch, Event *event)
{
	return true;
}


void Crystaloid::updateCrystal(Sprite *sprite, int num)
{
	string name = "b";
	name += to_string(num);
	name.append(".png");	
...
void Crystaloid::update(float dt)
{
			...
			for (auto &bonus : bonuses)
			{
				...
				if (checkPaddleCollision(bonus, player))
				{
					...
					case 2:
						...
						electron.velY /= 2;
						velocity /= 2;
						this->schedule(schedule_selector(Crystaloid::resetVelocity), 25);
...
bool Crystaloid::checkPaddleCollision(T1 &_A, T2 _B)
{
...
void Crystaloid::startedPosition()
{
	isStarted = false;
...
void Crystaloid::resetVelocity(float dt)
{
	this->unschedule(schedule_selector(Crystaloid::resetVelocity));
	velocity = 8;
}


void Crystaloid::createBorder()
{
	auto bh = 0.0f;
